...
## 'Tube' vial number
## 'CloneSporeID' an identifier for each clone-by-spore treatment
...
nll2 <- function(params, data, mod) {
    ...
    for (id in unique(data$CloneID)) {
        ...
        ## Otherwise, estimate the amount of algae that is remaining
            ...
            for (cid in unique(data$plate_treatment)){
            ...
            ## the issue, I think, is that if the data is already subsetted for the genotype, that subset of data doesn't include 
            ## the control values  - but that might not be the case. 
